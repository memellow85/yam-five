<template>
  <section class="wrapper-page">
    <article>
      <h3>
        <span class="yamicons mdi mdi-account-question-outline"></span>
        {{ $t('help.title_1') }}
      </h3>
    </article>
    <div class="main">
      <article class="wrapper-help">
        <p>{{ $t('help.text_1') }}</p>
        <div>
          <h4>{{ $t('help.title_2') }}</h4>
          <p>{{ $t('help.text_2') }}</p>
        </div>
        <div>
          <h4>{{ $t('help.title_3') }}</h4>
          <p>{{ $t('help.text_3') }}</p>
          <ul>
            <li>
              <p>
                <i>{{ $t('help.text_3_1_1') }}</i>
                {{ $t('help.text_3_1_2') }}
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.text_3_2_1') }}</i>
                {{ $t('help.text_3_2_2') }}
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.text_3_3_1') }}</i>
                {{ $t('help.text_3_3_2') }}
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.text_3_4_1') }}</i>
                {{ $t('help.text_3_4_2') }}
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.text_3_5_1') }}</i>
                {{ $t('help.text_3_5_2') }}
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.text_3_6_1') }}</i>
                {{ $t('help.text_3_6_2') }}
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.text_3_7_1') }}</i>
                {{ $t('help.text_3_7_2') }}
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.text_3_8_1') }}</i>
                {{ $t('help.text_3_6_8') }}
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.text_3_9_1') }}</i>
                {{ $t('help.text_3_9_2') }}
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.text_3_10_1') }}</i>
                {{ $t('help.text_3_10_2') }}
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.text_3_11_1') }}</i>
                {{ $t('help.text_3_11_2') }}
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.text_3_12_1') }}</i>
                {{ $t('help.text_3_12_2') }}
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.text_3_13_1') }}</i>
                {{ $t('help.text_3_13_2') }}
              </p>
            </li>
          </ul>
          <p>{{ $t('help.text_3_14') }}</p>
          <p>
            <i>{{ $t('help.text_3_15_1') }}</i>
          </p>
          <p>{{ $t('help.text_3_15_2') }}</p>
          <ul>
            <li>
              <p>
                <i>{{ $t('help.game_1') }}</i>
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.game_2') }}</i>
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.game_3') }}</i>
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.game_4') }}</i>
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.game_5') }}</i>
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.game_6') }}</i>
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.game_7') }}</i>
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.game_8') }}</i>
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.game_9') }}</i>
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.game_10') }}</i>
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.game_11') }}</i>
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.game_12') }}</i>
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.game_13') }}</i>
              </p>
            </li>
          </ul>
          <p>
            <i>{{ $t('help.text_3_16_1') }}</i>
          </p>
          <p>{{ $t('help.text_3_16_2') }}</p>
          <p>
            <i>{{ $t('help.text_3_17_1') }}</i>
          </p>
          <p>{{ $t('help.text_3_17_2') }}</p>
          <p>
            <i>{{ $t('help.text_3_18_1') }}</i>
          </p>
          <p>{{ $t('help.text_3_18_2') }}</p>
          <ul>
            <li>
              <p>
                <i>{{ $t('help.game_13') }}</i>
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.game_12') }}</i>
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.game_11') }}</i>
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.game_10') }}</i>
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.game_9') }}</i>
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.game_8') }}</i>
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.game_7') }}</i>
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.game_6') }}</i>
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.game_5') }}</i>
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.game_4') }}</i>
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.game_3') }}</i>
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.game_2') }}</i>
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.game_1') }}</i>
              </p>
            </li>
          </ul>
          <p>{{ $t('help.text_3_19_1') }}</p>
        </div>
        <div>
          <h4>{{ $t('help.title_4') }}</h4>
          <p>
            <i>{{ $t('help.text_4_1_1') }}</i>
            {{ $t('help.text_4_1_2') }}
          </p>
          <p>
            <i>{{ $t('help.text_4_2_1') }}</i>
            {{ $t('help.text_4_2_2') }}
          </p>
          <p>
            <i>{{ $t('help.text_4_3_1') }}</i>
            {{ $t('help.text_4_3_2') }}
          </p>
        </div>
        <div>
          <h4>{{ $t('help.title_5') }}</h4>
          <ul class="with-icon">
            <li>
              <span class="yamicons mdi mdi-cog-outline"></span>
              <p>{{ $t('help.text_5_1') }}</p>
            </li>
            <li>
              <span class="yamicons mdi mdi-view-grid-outline"></span>
              <p>{{ $t('help.text_5_2') }}</p>
            </li>
            <li>
              <span class="yamicons mdi mdi-chart-line-variant"></span>
              <p>{{ $t('help.text_5_3') }}</p>
            </li>
            <li>
              <span class="yamicons mdi mdi-help-circle-outline"></span>
              <p>{{ $t('help.text_5_4') }}</p>
            </li>
            <li>
              <span class="yamicons mdi mdi-sort-bool-ascending-variant"></span>
              <p>{{ $t('help.text_5_5') }}</p>
            </li>
            <li>
              <span
                class="yamicons mdi mdi-order-bool-ascending-variant"
              ></span>
              <p>{{ $t('help.text_5_6') }}</p>
            </li>
            <li>
              <span class="yamicons mdi mdi-target"></span>
              <p>{{ $t('help.text_5_7') }}</p>
            </li>
            <li>
              <span
                class="yamicons mdi mdi-sort-bool-descending-variant"
              ></span>
              <p>{{ $t('help.text_5_8') }}</p>
            </li>
            <li>
              <span class="yamicons mdi mdi-trash-can-outline"></span>
              <p>{{ $t('help.text_5_9') }}</p>
            </li>
            <li>
              <span class="yamicons mdi mdi-plus-box-outline"></span>
              <p>{{ $t('help.text_5_10') }}</p>
            </li>
          </ul>
        </div>
        <div>
          <h4>{{ $t('help.title_6') }}</h4>
          <p>{{ $t('help.text_6_1') }}</p>
          <ul>
            <li>
              <p>
                <i>{{ $t('help.text_6_1_1') }}</i>
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.text_6_1_2') }}</i>
              </p>
            </li>
          </ul>
          <p>{{ $t('help.text_6_2') }}</p>
          <ul>
            <li>
              <p>
                <i>{{ $t('help.text_6_2_1_1') }}</i>
                {{ $t('help.text_6_2_1_2') }}
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.text_6_2_2_1') }}</i>
                {{ $t('help.text_6_2_2_2') }}
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.text_6_2_3_1') }}</i>
                {{ $t('help.text_6_2_3_2') }}
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.text_6_2_4_1') }}</i>
                {{ $t('help.text_6_2_4_2') }}
              </p>
            </li>
            <li>
              <p>
                <i>{{ $t('help.text_6_2_5_1') }}</i>
                {{ $t('help.text_6_2_5_2') }}
              </p>
            </li>
          </ul>
          <p>{{ $t('help.text_6_3') }}</p>
        </div>
      </article>
    </div>
  </section>
</template>

<script>
import WsMixin from '@/mixins/ws'

export default {
  mixins: [WsMixin],
  middleware: ['authenticated'],
}
</script>

<style lang="scss" scoped>
section {
  p {
    @include margin(null null 1rem);
    line-height: 1.5;
  }
}
</style>
